...
namespace KerbalLiDAR
{
    ...
    public sealed class ModuleKerbalStarTracker : PartModule
    {
        ...
        [KSPField] public string udpHost = "127.0.0.1";
        [KSPField] public int udpPort = 49010;
        ...
        private void SendState(string reason, Quaternion rotation, float rate)
        {
            ...
            packet.Append("{\"type\":\"ksp_star_tracker\",\"version\":1,\"vesselId\":\"").Append(vessel.id.ToString("N"));
            packet.Append("\",\"sensorId\":\"").Append(ModuleKerbalRosSensorId.Resolve(part, "star_tracker"));
            ...
            try
            {
                ...
                var bytes = Encoding.UTF8.GetBytes(packet.ToString()); client.Send(bytes, bytes.Length);
            }
            catch (Exception ex)
            {
                if (client != null) client.Close(); client = null;
                if (Time.realtimeSinceStartup - lastWarning > 5)
                { lastWarning = Time.realtimeSinceStartup; Debug.LogWarning("[KerbalLiDAR] Star tracker UDP: " + ex.Message); }
            }
        }
