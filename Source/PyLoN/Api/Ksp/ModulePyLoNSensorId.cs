...
namespace PyLoN
{
    public sealed class ModulePyLoNSensorId : PartModule
    {
        ...
        public static string Resolve(Part target, string fallback)
        {
            if (target != null)
            {
                var identity = target.FindModuleImplementing<ModulePyLoNSensorId>();
        ...
        private HashSet<string> OtherSensorIds()
        {
            ...
            for (var index = 0; index < parts.Count; index++)
            {
                ...
                var identity = candidate.FindModuleImplementing<ModulePyLoNSensorId>();
