...
namespace PyLoN
{
    ...
    {
        private const string VesselProxyType = "pylon_active_vessel_proxy";
        private const string VesselProxyChunkType = "pylon_vessel_urdf_chunk";
        private const string VesselProxyClearType = "pylon_vessel_urdf_clear";
        ...
        {
            return activeVesselUrdfEnabled && udpEnabled && vessel != null &&
                vessel.parts != null && FlightGlobals.ActiveVessel == vessel;
        }
        ...
        private string BuildActiveVesselProxyBundle(out string modelId)
        {
            ...
            var namePrefix = "pylon_" + vesselId.Substring(0, 8);
            ...
            // Deployment, robotics, variants and rescaling can change geometry without
            // changing the part tree. Sample the current state on every refresh.
            ...
            var rootPositionInBase = FrameConversions.UnityVectorToRos(
                worldToBaseRotation * (rootPart.transform.position - vessel.CoM)
            );
            var rootRotationInBase = FrameConversions.UnityRotationToRos(
        ...
        )
        {
            ...
            for (var index = 1; index < orderedParts.Count; index++)
            {
                ...
                AppendFixedJoint(
                    ...
                    linkNames[parent],
                    linkNames[child],
                    PartMetricPoint(parent.transform, child.transform.position),
        ...
        private static void AppendProxyLink(StringBuilder urdf, Part vesselPart, string linkName)
        {
            ...
            var rosCenter = FrameConversions.UnityVectorToRos(bounds.Center);
        ...
        )
        {
            ...
            {
                var rosCenter = FrameConversions.UnityVectorToRos(geometry.Center);
                var rosRotation = FrameConversions.UnityRotationToRos(geometry.Rotation);
                // Proxy cylinders use Unity local Z, which maps to ROS X; URDF
                // cylinders use local Z. Apply this basis correction only to cylinders.
                if (geometry.Type == ProxyGeometryType.Cylinder)
                {
                    rosRotation *= Quaternion.FromToRotation(Vector3.forward, Vector3.right);
                }
                var rosRpy = RosQuaternionToRpy(rosRotation);
        ...
        {
            var rosPosition = FrameConversions.UnityVectorToRos(unityPosition);
            var rosRpy = RosQuaternionToRpy(FrameConversions.UnityRotationToRos(unityRotation));
        ...
        private static List<ProxyGeometry> BuildProxyGeometries(Part vesselPart)
        {
            ...
            AppendRendererProxyGeometries(geometries, vesselPart, int.MaxValue);
            if (geometries.Count > 0)
            {
                return LimitProxyGeometries(geometries, maximumGeometriesPerPart);
            }

            AppendColliderProxyGeometries(geometries, vesselPart, int.MaxValue);
            if (geometries.Count == 0)
            {
        ...
        private static List<ProxyGeometry> LimitProxyGeometries(List<ProxyGeometry> geometries, int maximum)
        {
            if (geometries.Count > maximum)
            {
                // Preserve the full extent of the remaining components instead of
                // silently dropping solar panels, trusses or other complex assemblies.
                var remainder = geometries.GetRange(maximum - 1, geometries.Count - maximum + 1);
                var bounds = EstimateProxyBounds(remainder);
                geometries.RemoveRange(maximum - 1, geometries.Count - maximum + 1);
                geometries.Add(new ProxyGeometry
                {
                    Type = ProxyGeometryType.Box,
                    Center = bounds.Center,
                    Rotation = Quaternion.identity,
                    Size = MaxVector(bounds.Size, Vector3.one * 0.01f)
                });
            }
            return geometries;
        }
        ...
        )
        {
            ...
            foreach (var renderer in renderers)
            {
                ...
                {
                    // Current local bounds include skeletal deformation. The shared
                    // mesh is the bind pose and cannot describe the deployed shape.
                    geometries.Add(CreateBoxGeometry(
                        vesselPart.transform, skinnedRenderer.transform,
                        skinnedRenderer.localBounds.center, skinnedRenderer.localBounds.size
                    ));
                }
            }
        }
        ...
        )
        {
            ...
            foreach (var collider in colliders)
            {
                if (
                    ...
                    || !collider.enabled
                    || collider.isTrigger
                    || FindPart(collider.transform) != vesselPart
                    || geometries.Count >= maximumGeometries
                )
                {
                    continue;
                ...
                {
                    // WheelCollider and mod-specific collider types still get a proxy.
                    geometries.Add(CreateBoxGeometry(
                        vesselPart.transform, null, collider.bounds.center, collider.bounds.size
                    ));
                }
        ...
        )
        {
            ...
            {
                Type = ProxyGeometryType.Box,
                Center = PartMetricPoint(partTransform, geometryTransform == null ? localCenter : geometryTransform.TransformPoint(localCenter)),
        ...
        )
        {
            ...
            var center = PartMetricPoint(partTransform, capsule.transform.TransformPoint(capsule.center));
        ...
        )
        {
            ...
            var center = PartMetricPoint(partTransform, meshTransform.TransformPoint(bounds.center));
        ...
        // URDF links are rigid frames in metres, not Unity frames with scale.
        private static Vector3 PartMetricPoint(Transform partTransform, Vector3 worldPoint)
        {
            return Quaternion.Inverse(partTransform.rotation) * (worldPoint - partTransform.position);
        }
        ...
        private static Vector3 RelativeScale(Transform partTransform, Transform geometryTransform)
        {
            ...
            return new Vector3(
                geometryTransform.TransformVector(Vector3.right).magnitude,
                geometryTransform.TransformVector(Vector3.up).magnitude,
                geometryTransform.TransformVector(Vector3.forward).magnitude
            );
        }

        private static Quaternion RelativeRotation(Transform partTransform, Transform geometryTransform)
        {
            return (Quaternion.Inverse(partTransform.rotation)
                * (geometryTransform == null ? Quaternion.identity : geometryTransform.rotation)).normalized;
        }
        ...
        private static ProxyBounds EstimateProxyBounds(IList<ProxyGeometry> geometries)
        {
            ...
            return new ProxyBounds
            {
                ...
                Size = MaxVector(maximum - minimum, Vector3.one * 0.01f)
            };
        }
