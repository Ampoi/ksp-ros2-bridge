...
namespace PyLoN
{
    ...
    public sealed class PyLoNPartIdOverlay : MonoBehaviour
    {
        ...
        private void CollectPartIds(Vessel vessel)
        {
            ...
            foreach (var part in vessel.parts)
            {
                ...
                for (var index = 0; index < part.Modules.Count; index++)
                {
                    ...
                    string kind = null;
                    string id = null;
                    var sensor = module as ModulePyLoNSensorId;
                    var motor = module as IPyLoNMotor;
                    if (sensor != null) { kind = sensor.sensorKind; id = ModulePyLoNSensorId.Resolve(part, kind); }
                    ...
                    if (id == null) id = PyLoNActuatorNames.For(kind, part, index);
                    AddPartId(part, id);
                }
            }
        }
