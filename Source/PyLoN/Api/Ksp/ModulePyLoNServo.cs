...
namespace PyLoN
{
    ...
    /// </summary>
    public sealed class ModulePyLoNServo : ModuleJointMotor, IPyLoNMotor
    {
        ...
        string IPyLoNMotor.JointType { get { return "revolute"; } }
        float IPyLoNMotor.StateRateHz { get { return stateRateHz; } }

        private string JointName
        {
            get { return PyLoNMotorNames.Resolve(motorName, "servo", part, this); }
        }
        ...
        protected override void OnJointInit(bool goodSetup)
        {
            ...
            if (!jointInitialized)
            {
                ...
                Debug.LogWarning("[PyLoN] ROS axial servo has no physical joint at node '" + drivenNodeName + "': " + JointName);
                return;
            }
            ...
            if (drivenPart == null)
            {
                ...
                Debug.LogWarning("[PyLoN] ROS axial servo top face has no attached part: " + JointName);
                return;
            }

            PyLoNMotorCollisions.EnableBetweenConnectedParts(part, drivenPart, joint);
        ...
        private void CalibrateDriveSign()
        {
            ...
            if (!driveSignCalibrated && Mathf.Abs(lastLogicalSpeedDegrees) > 1f && Mathf.Abs(measuredDelta) > 0.02f)
            {
                ...
                {
                    driveSign = -driveSign;
                    Debug.Log("[PyLoN] ROS axial servo automatically reversed its physical drive sign: " + JointName);
                }
        ...
        private void EnsureJointConfiguration()
        {
            ...
            if (configurationChanged)
            {
                ...
                joint.angularZMotion = ConfigurableJointMotion.Locked;
                joint.rotationDriveMode = RotationDriveMode.XYAndZ;
                Debug.Log("[PyLoN] ROS axial servo restored motor freedom after KSP joint rebuild: " + JointName);
            }
        ...
        private void LogJointDiagnostics(string reason)
        {
            if (joint == null)
            {
                Debug.LogWarning("[PyLoN] ROS axial servo " + reason + ": Unity joint is null, " + JointName);
                return;
            }

            var drive = joint.angularXDrive;
            Debug.Log(
                "[PyLoN] ROS axial servo " + reason + ": " + JointName +
                " node=" + drivenNodeName +
