...
namespace PyLoN
{
    public partial class ModulePyLoNLidar : PartModule
    {
        ...
        private void SendFlightTopicInactive()
        {
            ...
            AppendProperty(packetBuilder, "type", "pylon_lidar_inactive", true);
        ...
        private string BuildScanPacket(Vector3 origin, Vector3 forward, Vector3 up, int horizontalCount, int verticalCount)
        {
            ...
            var sensorPositionInPart = FrameConversions.UnityVectorToRos(partTransform.InverseTransformPoint(origin));
            var sensorRotationInPart = FrameConversions.UnityRotationToRos(
                ...
                for (var horizontal = 0; horizontal < horizontalCount; horizontal++)
                {
                    ...
                    if (hasHit)
                    {
                        ...
                        if (points != null)
                        {
                            AppendVector3OrNull(
                                points,
                                FrameConversions.UnityVectorToRos(worldToSensorRotation * (hit.point - origin)),
                                true
                            );
                        }
                    }
                    ...
                    if (directions != null)
                    {
                        AppendVector3OrNull(
                            directions,
                            FrameConversions.UnityVectorToRos(worldToSensorRotation * direction),
                            true
                        );
                    }
            ...
            packetBuilder.Length = 0;
            packetBuilder.Append('{');
            AppendProperty(packetBuilder, "type", "pylon_lidar_scan", true);
            AppendProperty(packetBuilder, "version", JsonVersion, false);
            AppendProperty(packetBuilder, "mode", Is3DMode() ? "3D" : "2D", false);
            AppendProperty(packetBuilder, "sensorId", ResolveSensorId(), false);
            AppendProperty(packetBuilder, "vessel", vessel != null ? vessel.vesselName : "", false);
        ...
        {
            return ModulePyLoNSensorId.Resolve(part, "");
        }
