...
namespace PyLoN
{
    ...
    /// flight-joint lifecycle as ModulePyLoNServo. In the editor this is
    ...
    /// </summary>
    public sealed partial class ModulePyLoNLinearMotor : ModuleJointMotor, IPyLoNMotor
    {
        ...
        Part IPyLoNMotor.MotorPart { get { return part; } }
        string IPyLoNMotor.JointName { get { return JointName; } }
        string IPyLoNMotor.JointType { get { return "prismatic"; } }
        float IPyLoNMotor.StateRateHz { get { return stateRateHz; } }

        private string JointName
        {
            get { return PyLoNMotorNames.Resolve(motorName, "linear", part, this); }
        }
        ...
        protected override void OnModuleStart(StartState state)
        {
            ...
            {
                PyLoNMotorUdpManager.Register(this);
            }
        ...
        protected override void OnJointInit(bool goodSetup)
        {
            ...
            if (!jointInitialized)
            {
                ...
                Debug.LogWarning("[PyLoN] ROS linear actuator has no physical joint at node '" + drivenNodeName + "': " + JointName);
                return;
            }
            ...
            if (drivenPart == null)
            {
                ...
                Debug.LogWarning("[PyLoN] ROS linear actuator top end has no attached part: " + JointName);
                return;
            }
            ...
            {
                if (slider != null)
                    PyLoNMotorCollisions.EnableBetweenConnectedParts(part, drivenPart, slider);
            }
        ...
        {
            PyLoNMotorUdpManager.Unregister(this);
        }
        ...
        void IPyLoNMotor.ApplyRosCommand(PyLoNMotorCommand command)
        {
            if (command == null)
            {
                return;
        ...
        string IPyLoNMotor.BuildStatePacket()
        {
        ...
        private void CalibrateDriveSign()
        {
            ...
            if (!driveSignCalibrated && Mathf.Abs(lastLogicalSpeed) > 0.01f && Mathf.Abs(measuredDelta) > 0.0001f)
            {
                ...
                {
                    driveSign = -driveSign;
                    Debug.Log("[PyLoN] ROS linear actuator automatically reversed its physical drive sign: " + JointName);
                }
        ...
        private void EnsureVisuals()
        {
            ...
            if (string.IsNullOrEmpty(movingModelPath))
            {
                if (part.FindModelTransform(movingTransformName) == null)
                    Debug.LogError("[PyLoN] Linear actuator model is missing " + movingTransformName);
                return;
            }

            var target = PyLoNMotorVisuals.EnsureTransform(part, movingTransformName);
            if (target == null)
            {
                Debug.LogError("[PyLoN] ROS linear actuator could not create its moving transform.");
                return;
            }

            PyLoNMotorVisuals.AddStockModel(
                part,
                target,
                movingModelPath,
                movingModelPosition,
