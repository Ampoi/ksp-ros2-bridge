...
from ksp_ros2_interfaces.msg import WheelState, WheelCommand, ControlAuthorityState, ControlAuthorityCommand, VesselLifecycle
from ksp_vehicle_control.application.lease import LeaseCoordinator
from position_estimator.core import transform_points
...
class Rover(Node):
    def __init__(self):
        ...
        identity=TransformStamped(); identity.header.frame_id='map'; identity.child_frame_id='rover_odom'; identity.transform.rotation.w=1.
        self.static_tf.sendTransform(identity)
        self.odom_pub=self.create_publisher(Odometry,'/mun_rover/odom',10)
        self.pose_pub=self.create_publisher(Odometry,'/mun_rover/odom_3d',10)
        self.map_pub=self.create_publisher(OccupancyGrid,'/mun_rover/map',self.qos)
        self.cloud_pub=self.create_publisher(PointCloud2,'/mun_rover/points',qos_profile_sensor_data)
        self.ground_pub=self.create_publisher(PointCloud2,'/mun_rover/ground',qos_profile_sensor_data)
        self.obstacle_pub=self.create_publisher(PointCloud2,'/mun_rover/obstacles',qos_profile_sensor_data)
        self.status_pub=self.create_publisher(String,'/mun_rover/status',self.qos)
        self.marker_pub=self.create_publisher(Marker,'/mun_rover/status_marker',10)
        self.trail_pub=self.create_publisher(Path,'/mun_rover/trajectory',10)
        self.geometry_pub=self.create_publisher(String,'/mun_rover/geometry',self.qos)
        ...
        self.footprint_pubs=[self.create_publisher(Polygon,f'/mun_rover/{name}_costmap/footprint',10) for name in ('local','global')]
        self.nav_client=ActionClient(self,NavigateToPose,'/mun_rover/navigate_to_pose',callback_group=self.group)
        ...
        self.create_service(Trigger,'/mun_rover/reset',self.reset_service)
        ...
        self.create_subscription(Twist,'/mun_rover/cmd_vel',self.command,10)
        self.param_client=AsyncParameterClient(self,'/mun_rover/planner_server',callback_group=self.group)
        self.sequence=0; self.epoch=None; self.vessel_id=''; self.vessel_active=False
        self.lease=LeaseCoordinator('mun_rover',0.4)
    ...
    def lifecycle(self,msg):
        ...
        self.lease.observe_vessel(self.vessel_id,self.vessel_active)
    ...
    def publish_pose(self,stamp):
        ...
        tf=TransformStamped(); tf.header=Header(stamp=stamp,frame_id='rover_odom'); tf.child_frame_id='rover_base_footprint'
        ...
        child=TransformStamped(); child.header=Header(stamp=stamp,frame_id='rover_base_footprint'); child.child_frame_id='rover_base_link'
        ...
        msg=Odometry(); msg.header=Header(stamp=stamp,frame_id='rover_odom'); msg.child_frame_id='rover_base_footprint'
        ...
        full=Odometry(); full.header=msg.header; full.child_frame_id='rover_base_link'
