...
class PositionEstimatorNode(Node):
    ...
    def __init__(self) -> None:
        super().__init__("pylon_demo_position_estimator")
        ...
        self.declare_parameter("odom_topic", "/pylon/position_estimator/odom")
        self.declare_parameter("status_topic", "/pylon/position_estimator/status")
        self.declare_parameter("error_topic", "/pylon/position_estimator/error")
        ...
        self.declare_parameter("odom_frame", "pylon_lidar_odom_3d")
        self.declare_parameter("base_frame", "pylon_estimator_base_link")
        ...
        self.create_subscription(VesselLifecycle, '/ksp_vessel/lifecycle', self.receive_lifecycle,
    ...
    def receive_lifecycle(self, message):
        ...
        key = (message.vessel_id, message.generation) if active else None
        if key == self.lifecycle_key:
            return
        self.lifecycle_key = key
        self.previous_points = self.previous_stamp = None
        self.previous_delta = self.pose = np.eye(4, dtype=np.float64)
        self.ground_truth.clear()
        self.estimated_origin = self.truth_origin = None
        self.estimated_origin_quat = self.truth_origin_quat = None
        self.sequence = 0
    ...
    def publish_state(
        ...
        # bridge's pylon_ground_truth_enu -> base_link transform.
