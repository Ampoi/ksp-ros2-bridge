...
/ksp_vessel/lidar_3d/<sensor_id>/points
  ...
  -> /pylon/position_estimator/odom + pylon_lidar_odom_3d -> pylon_estimator_base_link
  -> console echo + /pylon/position_estimator/error (vs ground truth, 評価のみ)
```
...
colcon build --packages-select pylon_demo_position_estimator
...
```bash
ros2 topic list | grep -E 'lidar_3d|pylon_demo_position_estimator'
...
RViz2で確認する場合はFixed Frameを`pylon_lidar_odom_3d`にし、Odometry表示のTopicを`/pylon/position_estimator/odom`にします。推定のTFは`pylon_lidar_odom_3d -> pylon_estimator_base_link`という独立ツリーのため、bridgeが配信する真値TF（`pylon_ground_truth_enu -> base_link`）とは競合しません。

起動中のターミナルには約2秒おきに推定位置と誤差が出ます。

```text
[pylon_demo_position_estimator_node-1] est=[12.34 0.56 -0.12] drift=0.423m att_err=1.35deg rmse=0.112 matches=642
```

誤差だけを別ターミナルで見る場合:

```bash
ros2 topic echo /pylon/position_estimator/error
ros2 topic echo /pylon/position_estimator/status
```

```bash
ros2 topic echo /pylon/position_estimator/odom
```

## 誤差の見方

- Odometryは起動時の位置・姿勢を原点`(0,0,0)`として積算します。一方Ground Truthは`pylon_ground_truth_enu`原点の絶対座標なので、そのまま引き算しても意味がありません。このため誤差は**開始時点合わせ**で測ります: 初回採点時の推定値と真値をそれぞれの原点として記録し、そこからの変位ベクトルの差を`position_error` [m]、相対姿勢の差を`attitude_error_deg` [deg]として出します。
...
既定値は[`config/pylon_demo_position_estimator.yaml`](config/pylon_demo_position_estimator.yaml)にあります。コピーして変更し、次のように読み込めます。

```bash
ros2 launch pylon_demo_position_estimator pylon_demo_position_estimator.launch.py \
  config_file:=/absolute/path/to/my_pylon_demo_position_estimator.yaml
```
...
| `odom_topic` | 推定Odometryの出力先 | `/pylon/position_estimator/odom` |
| `error_topic` | 真値対比の誤差JSONの出力先 | `/pylon/position_estimator/error` |
...
| `odom_frame` / `base_frame` | 推定TFの親/子フレーム | `pylon_lidar_odom_3d` / `pylon_estimator_base_link` |
...
| Publish | `/pylon/position_estimator/odom` | `nav_msgs/msg/Odometry` |
| Publish | `/pylon/position_estimator/status` | `std_msgs/msg/String` (JSON) |
| Publish | `/pylon/position_estimator/error` | `std_msgs/msg/String` (JSON) |
| Publish | TF `pylon_lidar_odom_3d -> pylon_estimator_base_link` | `tf2_msgs/msg/TFMessage` |
