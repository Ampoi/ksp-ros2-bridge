...
class RelativeContractTests(unittest.TestCase):
    ...
    def sample(self):
        ...
        m.header.frame_id = 'pylon_debris_inertial'
    ...
    def test_camera_capture_continues_after_first_turn_and_uses_image_time(self):
        ...
        p = PoseStamped(); p.header.frame_id = 'pylon_debris_inertial'
    ...
    def test_old_orbit_does_not_resume_after_sensor_loss_or_epoch_change(self):
        node = self.guidance
        node.run_started = True
        node.control()
        self.assertEqual(node.halt_reason, 'sensor_timeout')
        node._state_is_fresh = lambda now: True
        node.control()
        self.assertEqual(node.setpoint_publisher.publish.call_args.args[0].mode, ControlSetpoint.MODE_IDLE)
        node.halt_reason = ''
        node.receive_lifecycle(VesselLifecycle(vessel_id='chaser', origin_sequence=8, state=1))
        self.assertEqual(node.halt_reason, 'session_changed')
