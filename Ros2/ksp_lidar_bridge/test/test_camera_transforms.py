...
try:
    ...
    KerbalLidarUdpBridge = None


@unittest.skipIf(KerbalLidarUdpBridge is None, 'source ROS Jazzy to test camera TF')
class CameraTransformTests(unittest.TestCase):
    def test_moving_camera_preserves_each_image_timestamp(self):
        ...
        node.model_frame_for_packet.return_value = 'camera_part'
        ...
        for seconds, rotation in [(1, [0., 0., 0., 1.]),
            ...
            KerbalLidarUdpBridge.sensor_frame_for_packet(
                node, packet, 'front_camera', Time(sec=seconds), 'camera_optical_frame', dynamic=True)
    ...
    def test_fixed_lidar_still_uses_static_snapshot(self):
        node = Mock()
        node.args = SimpleNamespace(frame_prefix='test')
        node.model_frame_for_packet.return_value = 'lidar_part'
        node.static_sensor_transforms = {}
        packet = dict(coordinateFrame='ros_sensor', framePosition=[0., 0., 0.1],
                      frameRotation=[0., 0., 0., 1.])
        for second in (1, 2):
            KerbalLidarUdpBridge.sensor_frame_for_packet(node, packet, 'lidar', Time(sec=second))
