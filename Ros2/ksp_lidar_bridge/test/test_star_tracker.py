...
from ksp_lidar_bridge.star_tracker_packets import star_tracker_from_packet


def packet(**changes):
    data = dict(type='ksp_star_tracker', version=1, vesselId='a'*32,
...
class PacketTests(unittest.TestCase):
    def test_valid_and_every_loss_state(self):
        from ksp_lidar_bridge.star_tracker_packets import REASONS
...
class BridgeTests(unittest.TestCase):
    def setUp(self):
        ...
        from ksp_lidar_bridge.star_tracker_bridge import StarTrackerBridgeMixin
        self.published = {}
        self.removed = []
        class Fake(StarTrackerBridgeMixin):
            pass
        self.node = Fake()
    ...
    def test_loss_never_publishes_attitude_or_reuses_last_quaternion(self):
        self.node.publish_star_tracker(packet())
        self.node.publish_star_tracker(packet(sequence=2, valid=False, reason='sun_exclusion', orientation=None))
        self.node.publish_star_tracker(packet())  # delayed good packet
    ...
    def test_timeout_heartbeat_recovery_and_bounded_cleanup(self):
        with patch('ksp_lidar_bridge.star_tracker_bridge.time.monotonic', return_value=10.):
            self.node.publish_star_tracker(packet())
        with patch('ksp_lidar_bridge.star_tracker_bridge.time.monotonic', return_value=13.):
            self.node.expire_star_trackers()
            self.node.expire_star_trackers()
            self.node.publish_star_tracker(packet(sequence=2))
        states = self.published[self.prefix + '/state']
        ...
        with patch('ksp_lidar_bridge.star_tracker_bridge.time.monotonic', return_value=50.):
            self.node.expire_star_trackers()
    ...
    def test_vessel_switch_and_independent_sensors(self):
        self.node.publish_star_tracker(packet())
        self.node.publish_star_tracker(packet(sensorId='star_tracker_2', sequence=8))
        ...
        self.node.publish_star_tracker(packet(sequence=3))
        self.assertEqual(self.published[self.prefix + '/state'][-1].reason, 'inactive')
        self.node.publish_star_tracker(packet(vesselId='c'*32, sessionId='d'*32))
...
class UdpRosTests(unittest.TestCase):
    def test_udp_to_dds_valid_loss_and_no_attitude_on_loss(self):
        ...
        bridge = KerbalLidarUdpBridge(parse_args(['--host', '127.0.0.1', '--port', '0', '--disable-ground-truth']))
        ...
        try:
            ...
            while not attitudes and time.monotonic() < deadline:
                seq += 1
                sender.sendto(json.dumps(packet(sequence=seq)).encode(), endpoint)
            ...
            seq += 1
            sender.sendto(json.dumps(packet(sequence=seq, valid=False, reason='occluded', orientation=None)).encode(), endpoint)
