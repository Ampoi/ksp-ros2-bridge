...
from pylon_bridge.docking_packets import (
    ACTION_RELEASE,
...
class DockingPacketTests(unittest.TestCase):
    def test_parses_manifest(self):
        packet = {
            "type": "pylon_docking_port_manifest",
            "version": 1,
    ...
    def test_rejects_duplicate_manifest_names_after_normalization(self):
        packet = {
            "type": "pylon_docking_port_manifest", "version": 1,
            "ports": [
    ...
    def test_parses_state_without_partner(self):
        state = docking_port_state_from_packet({
            "type": "pylon_docking_port_state", "version": 1,
    ...
    def test_encodes_typed_actions(self):
        packet = json.loads(encode_docking_port_command("Docking Port 4", ACTION_SELECT_CAMERA, 7, "vessel", "controller", "lease"))
        ...
        self.assertEqual(json.loads(encode_docking_port_command("port", ACTION_RELEASE, 8, "vessel", "controller", "lease"))["action"], ACTION_RELEASE)

    def test_rejects_unknown_action_and_invalid_sequence(self):
        with self.assertRaisesRegex(ValueError, "unsupported"):
            encode_docking_port_command("port", 99, 1, "vessel", "controller", "lease")
        with self.assertRaisesRegex(ValueError, "sequence"):
            encode_docking_port_command("port", ACTION_RELEASE, 0, "vessel", "controller", "lease")
        with self.assertRaisesRegex(ValueError, "name"):
            encode_docking_port_command("", ACTION_RELEASE, 1, "vessel", "controller", "lease")
