...
from pylon_bridge.packet_conversion import (
    chunked_vectors,
    decode_datagram,
    expired_topic_names,
...
class DatagramTests(unittest.TestCase):
    def test_decodes_json_object(self):
        self.assertEqual(
            decode_datagram(b'{"type":"pylon_lidar_scan","version":1}'),
            {"type": "pylon_lidar_scan", "version": 1},
        )
...
class VectorTests(unittest.TestCase):
    ...
    def test_reads_unsupported_nested_vectors(self):
        with self.assertRaises(ValueError):
            list(chunked_vectors([[1,2,3]]))

    def test_keeps_alignment_when_nested_vectors_start_with_null(self):
        with self.assertRaises(ValueError):
            list(chunked_vectors([None,[1,2,3]]))
...
class PointConversionTests(unittest.TestCase):
    ...
    def test_uses_unsupported_nested_hit_points(self):
        with self.assertRaises(ValueError):
            points_from_packet(self.packet(points=[[1,2,3]]))
    ...
    def test_calculates_points_from_unsupported_nested_directions(self):
        with self.assertRaises(ValueError):
            points_from_packet(self.packet(directions=[[1,0,0]]))
...
class NameTests(unittest.TestCase):
    ...
    def test_prefers_part_name(self):
        ...
        with self.assertRaises(ValueError):
            packet_sensor_id(packet)

    def test_sanitizes_explicit_lidar_name(self):
        packet = {"sensorId": "Front LiDAR #1"}
    ...
    def test_builds_full_topic_from_part_name_and_mode(self):
        self.assertEqual(
            lidar_topic_from_packet({"sensorId": "Front LiDAR", "mode": "2D"}),
            "/ksp_vessel/lidar_2d/front_lidar/scan",
        )
        self.assertEqual(
            lidar_topic_from_packet(
                {"sensorId": "roof_lidar", "mode": "3D"},
                "/robot/sensors/",
            ),
