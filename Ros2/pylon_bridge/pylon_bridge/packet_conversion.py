...
def packet_sensor_id(packet: Dict[str, Any]) -> str:
    """Return the canonical, explicitly configured sensor identity."""
    value = packet.get("sensorId")
...
def chunked_vectors(values: Any) -> Iterable[Vector3]:
    """Yield vectors from the flat PyLoN wire array."""
    if not isinstance(values, list) or not values:
        return
    if any(isinstance(value, (list, tuple)) for value in values):
        raise ValueError("vector arrays must be flat")
