...
class StarTrackerService:
    """Star tracker adapter composed by the PyLoN ROS node."""
    ...
    def publish_star_tracker(self, packet):
        try:
            ...
            self.bridge.get_logger().warning(f'Dropped invalid star tracker packet: {exc}')
            return
        if not hasattr(self.bridge, 'star_trackers'):
            self.bridge.star_trackers = {}
        key = data.sensor_id
        previous = self.bridge.star_trackers.get(key)
        ...
        if previous is not None:
            ...
            if old.session_id == data.session_id and data.sequence <= old.sequence:
                return
        active = getattr(self.bridge, 'active_vessel_id', '')
        if active and active != data.vessel_id:
            return
        if previous is None and len(self.bridge.star_trackers) >= 128:
            self.bridge.get_logger().warning('Star tracker publisher limit reached')
            return
        now = time.monotonic()
        self.bridge.star_trackers[key] = (data, now, False)
        stamp = self.bridge.flight.stamp_for_packet(packet)
        self.emit_star_tracker(data, stamp)

    def emit_star_tracker(self, data, stamp):
        prefix = f'{self.bridge.topic_prefix}/star_tracker/{data.sensor_id}'
        ...
        publisher = self.bridge.sensors.publisher_for(prefix + '/state', 'StarTrackerState', StarTrackerState)
        ...
        if data.valid:
            ...
            publisher = self.bridge.sensors.publisher_for(prefix + '/attitude', 'QuaternionStamped', QuaternionStamped)
            if publisher is not None:
                publisher.publish(attitude)

    def expire_star_trackers(self):
        now = time.monotonic()
        for key, (data, seen, stale) in list(getattr(self.bridge, 'star_trackers', {}).items()):
            active = getattr(self.bridge, 'active_vessel_id', '')
            switched = bool(active and active != data.vessel_id)
            if switched or now - seen > self.bridge.args.topic_timeout_sec:
                ...
                self.emit_star_tracker(invalid, self.bridge.get_clock().now().to_msg())
                self.bridge.star_trackers[key] = (data, seen, True)
                if now - seen > max(30., self.bridge.args.topic_timeout_sec * 3):
                    prefix = f'{self.bridge.topic_prefix}/star_tracker/{key}'
                    for suffix in ('/state', '/attitude'):
                        self.bridge.sensors.remove_publisher(prefix + suffix, 'star tracker expired')
                    del self.bridge.star_trackers[key]
